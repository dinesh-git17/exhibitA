...
# Covers iOS/Xcode toolchain and Python/FastAPI backend directories
EXCLUDE_DIRS=(
    ".git"
    ".claude"
    "build"
...
# Includes iOS binary formats (car, ipa, mobileprovision),
# Python bytecode (pyc, pyo), SQLite databases (db, db-wal, db-shm),
# and signature PNGs stored as blobs
BINARY_EXTENSIONS=(
...
scan_codebase() {
    ...
    # Exclusions cover iOS toolchain (DerivedData, xcuserdata, .swiftpm)
    # and Python backend (.venv, __pycache__, .mypy_cache, .ruff_cache)
